var policy = function() {
  ...
  function p1(tx) {
    ...
    for (var i = 0;i < len;i++) {
      var node = as[i];
      if (node.id === "innerHTML" && JAM.instanceof(node.obj, _HTMLElement) && __RegExp_prototype_test_call_bind__RegExp_prototype_test_(/^\s*(audio|frame|iframe|img|input|source|video)/i, node.value)) {
        commit = false;
        break;
      }
    }
  ...
  Object.freeze(p14);
  return {p1:p1, p10:p10, p13:p13, p15:p15, p14:p14, pFull:pFull, woven:true};
}()
